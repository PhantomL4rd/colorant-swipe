...
export async function renderShareImage(dyes: Dye[]): Promise<Blob> {
  ...
  ctx.font = `bold 56px ${jpFont}`;
  ctx.fillText('あなたへのおすすめ 4 色', W / 2, 430);

  ctx.fillStyle = '#555577';
  ctx.font = `400 36px ${jpFont}`;
  ctx.fillText('推しカララント、見つけよう。', W / 2, 495);

  ctx.fillStyle = '#F56496';
  ctx.font = `600 30px ${jpFont}`;
  ctx.fillText('カララントスワイプ', W / 2, 555);
...
export async function shareResult(dyes: Dye[], url: string, text: string): Promise<void> {
  const blob = await renderShareImage(dyes);
